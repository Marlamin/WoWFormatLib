...
namespace WoWFormatLib.FileReaders
{
    public static class WDBReader
    {
        ...
        public static WDBCache Read(string filename, string patch)
        {
            ...
            var targetBuild = new WDBBuild();
            var splitBuild = patch.Split('.');
            if (splitBuild.Length == 4)
            {
                targetBuild.version = patch;
            ...
            using (var bin = new BinaryReader(ms))
            {
                ...
                switch (wdb.identifier)
                {
                    ...
                    case "WNPC": // NPC
                    case "WPTN": // Petition
                        Console.WriteLine("Skipping unsupported cache file: " + wdb.identifier);
                        break;
                    default:
